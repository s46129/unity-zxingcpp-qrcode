...
With **Use Gpu Readback** on (the default) each frame is converted to Gray8, flipped and downscaled by `Gray8TextureReadback` on the GPU and read back asynchronously; the main thread only asks `CanAcceptFrame`, issues the blit, and copies the finished R8 frame into an `ArrayPool<byte>` buffer when the readback lands. The stats label shows `gpu` on this path. Because the readback already downscales, the decoder runs with `DownscaleFactor` 1 and the runner multiplies the reported corners by **Downscale Factor** before showing them. Where `Gray8TextureReadback.IsSupported` is false, or with the toggle off, the runner uses the CPU path below and the label shows `cpu`.

On the CPU path each accepted frame is converted from RGBA to Gray8 on the main thread (integer Rec. 601 luma) into an `ArrayPool<byte>` buffer, then flipped with `Gray8RowOrder.FlipVertically` because `WebCamTexture` rows start at the displayed bottom like every Unity texture. The conversion runs only after the scanner accepts the frame, so frames dropped by the scan interval cost nothing beyond the submit call.
